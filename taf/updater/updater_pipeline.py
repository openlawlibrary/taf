...
class AuthenticationRepositoryUpdatePipeline(Pipeline):
    def __init__(
        ...
        super().__init__(
            steps=[
                ...
                (
                    self.check_if_local_repos_match_lvc,
                    RunMode.UPDATE,
                    self.should_run_step_default,
                ),
                (
    ...
    def check_if_local_repos_match_lvc(self):
        """
        Check if the local auth repo and target repos on disk still match the
        state recorded in last_validated_data. If they do, set
        local_repos_consistent=True so that check_if_repo_is_synced_with_remote
        can potentially skip the full update when there are no upstream changes.

        If lvc data is missing, or any repo's HEAD does not match, we leave
        ...
        try:
            ...
            for repo_name, lvc_entry in self.state.last_validated_data.items():
                ...
                if auth_repo.get_target(repo_name, Commitish.from_hash(lvc_entry)):
                    return UpdateStatus.SUCCESS

            for repo_name, repo in self.state.repos_on_disk.items():
                lvc_entry = self.state.last_validated_data.get(repo_name)
                if lvc_entry is None:
                    # repo was excluded from a previous partial update
                    continue
    ...
    def determine_start_commits(self):
        ...
        try:
            ...
            if self.local_repos_consistent and not self.state.is_partially_updated:
                return UpdateStatus.SUCCESS
    ...
    def _collect_commits_with_old_head(self, repository, branch, old_head):
        if not self.only_validate:
            fetched_commits = repository.all_commits_on_branch(
                branch=f"origin/{branch}"
            )
            if old_head in fetched_commits:
                commits = fetched_commits[fetched_commits.index(old_head) + 1 :]
            else:
                commits = repository.all_commits_since_commit(old_head, branch)
                for commit in fetched_commits:
                    if commit not in commits:
                        commits.append(commit)
        else:
            commits = repository.all_commits_since_commit(old_head, branch)
        commits.insert(0, old_head)
        return commits

    def _collect_initial_branch_commits(self, repository, branch, local_branch_exists):
        if local_branch_exists:
            commits = repository.all_commits_on_branch(branch=branch, reverse=True)
        else:
            commits = []
        try:
            fetched_commits = repository.all_commits_on_branch(
    ...
    def _fetch_commits_for_branch(self, repository, branch, old_head):
        ...
        if old_head is not None:
            commits = self._collect_commits_with_old_head(repository, branch, old_head)
        else:
            commits = self._collect_initial_branch_commits(
                repository, branch, local_branch_exists
            )
    ...
    def get_target_repositories_commits(self):
        ...
        with ThreadPoolExecutor() as executor:
            future_to_branch = {
                executor.submit(
                    self._fetch_commits_for_branch,
                    repository,
                    branch,
    ...
    def validate_target_repositories(self):
        ...
        try:
            repositories = self.state.temp_target_repositories
            result = TargetValidator(
                self._get_law_steps(),
                self.state.fetched_commits_per_target_repos_branches,
                start=self._get_last_validated_target_pointers(),
                is_unauthenticated_allowed=lambda repo_name, auth_commit: self._is_unauthenticated_allowed_at(
                    repositories[repo_name], auth_commit
                ),
                auth_repo_name=self.state.users_auth_repo.name,
                get_commit_date=self.state.users_auth_repo.get_commit_date,
            ).validate()

            self.state.validated_auth_commits = result.validated_auth_commits
            self.state.validated_commits_per_target_repos_branches.update(
                result.validated_commits_per_repo_branch
            )
            self.state.last_validated_data_per_repositories.update(
                {
                    repo_name: {"commit": pointer.commit, "branch": pointer.branch}
                    for repo_name, pointer in result.last_validated_per_repo.items()
                }
            )
    ...
    def _get_law_steps(self) -> List[LawStep]:
        """
        For each authentication commit to validate, the branch and commit that it
        declares for each target repository that has a target file at that commit.
        """
        steps = []
        for auth_commit in self.state.all_targets_auth_commits:
            targets = {}
            for repository in self.state.temp_target_repositories.values():
                commits_data = self.state.targets_data_by_auth_commits.get(
                    repository.name, {}
                )
                if auth_commit not in commits_data:
                    continue
                target_data = commits_data[auth_commit]
                targets[repository.name] = TargetPointer(  # type: ignore[call-arg]
                    branch=target_data.get("branch", repository.default_branch),
                    commit=Commitish.from_hash(target_data["commit"]),
                )
            steps.append(
                LawStep(auth_commit=auth_commit, targets=targets)  # type: ignore[call-arg]
            )
        return steps

    def _get_last_validated_target_pointers(self) -> Dict[str, TargetPointer]:
        """
        Branch and commit of each target repository as of its last validated
        authentication commit. Repositories that were never validated are omitted.
        """
        pointers = {}
        for repository in self.state.temp_target_repositories.values():
            if repository.name not in self.state.targets_data_by_auth_commits:
                continue
            target_data = self.state.targets_data_by_auth_commits[repository.name].get(
                self._get_last_validated_commit(repository.name), {}
            )
            commit = Commitish.from_hash(target_data.get("commit"))
            if commit is None:
                continue
            branch = target_data.get("branch")
            if branch is None:
                branch = repository.default_branch
            pointers[repository.name] = TargetPointer(  # type: ignore[call-arg]
