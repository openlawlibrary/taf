...
def list_targets(
    ...
    repositories_data = auth_repo.sorted_commits_and_branches_per_repositories(top_commit)
    ...
    for repo_name, repo_data in repositories_data.items():
        ...
        repo_output["unauthenticated-allowed"] = repo.custom.get("allow-unauthenticated-commits", False)
                ...
                if not repo.branch_exists(branch, include_remotes=False):
                    ...
                    if not is_synced_with_remote:
                        ...
                        if branch in repo.branches_containing_commit(last_signed_commit):
                            ...
                            repo_output["unsigned"] = top_commit in repo.all_commits_since_commit(last_signed_commit, branch)
