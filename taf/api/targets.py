...
@check_if_clean
def taf_status(path: str, library_dir: Optional[str] = None) -> None:
    """
    ...
    # Load repositories and target repositories data
    repositoriesdb.load_repositories(auth_repo)
    target_repositories = repositoriesdb.get_deduplicated_repositories(auth_repo)
    repositories_data = auth_repo.sorted_commits_and_branches_per_repositories(
        top_commit
    ...
    print(f"Authentication Repository: {path}")
    ...
    # Print target repositories status
    print(json.dumps(output, indent=4))
