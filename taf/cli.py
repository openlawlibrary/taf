...
@cli.command()
@click.option(
    "--repo-path", default="repository", help="Authentication repository's path"
)
@click.option("--keystore", default=None, help="Path of the keystore file")
@click.option(
    "--keys-description",
    default=None,
    help="A dictionary containing information about the keys or a path"
    " to a json file which which stores the needed information",
)
@click.option(
    "--commit-msg",
    default=None,
    help="Commit message to be used in case the changes"
...
def add_targets(repo_path, keystore, keys_description, commit_msg, scheme):
    developer_tool.register_target_files(
        repo_path, keystore, keys_description, commit_msg, scheme
    )
...
@click.option(
    "--file-path", help="Target file's path, relative to the targets directory"
)
...
def add_target_file(repo_path, file_path, keystore, keys_description, scheme):
    developer_tool.register_target_file(
        repo_path, file_path, keystore, keys_description, scheme
    )
...
@click.option(
    "--targets-dir",
    default="targets",
    help="Directory where the target " "repositories are located",
)
...
@click.option(
    "--repo-path",
    default="repository",
    help="Location of the authentication repository",
)
@click.option(
    "--targets-dir",
    default="targets",
...
@click.option(
    "--targets-rel-dir",
    default=None,
    help=" Directory relative to which urls "
...
@click.option(
    "--custom",
    default=None,
    help="A dictionary containing custom "
...
def build_auth_repo(
    ...
    developer_tool.build_auth_repo(
        repo_path,
        targets_dir,
        namespace,
        targets_rel_dir,
...
@click.option(
    "--commit-msg",
    default=None,
    help="Commit message. If provided, the " "changes will be committed automatically",
)
@click.option(
    "--test",
    is_flag=True,
    default=False,
    help="Indicates if the created repository " "is a test authentication repository",
)
def create_repo(repo_path, keystore, keys_description, commit_msg, test):
    developer_tool.create_repository(
        repo_path, keystore, keys_description, commit_msg, test
    )
...
@click.option(
    ...
    is_flag=True,
    default=True,
    help="Indicates if changes should be committed",
)
@click.option(
    "--test",
    is_flag=True,
...
def generate_repositories_json(
    repo_path, targets_dir, namespace, targets_rel_dir, custom
):
...
@click.option(
    "--role",
    default="timestamp",
    help="Metadata role whose expiration date should be " "updated",
)
@click.option(
    "--start-date",
    default=datetime.datetime.now(),
    help="Date to which the intercal is added",
    type=ISO_DATE,
)
@click.option(
    "--interval", default=None, help="Time interval added to the start date", type=int
)
@click.option(
    "--commit-msg",
    default=None,
...
def update_expiration_date(
    repo_path, keystore, keys_description, role, start_date, interval, commit_msg
):
    developer_tool.update_metadata_expiration_date(
        repo_path, keystore, keys_description, role, start_date, interval, commit_msg
    )
...
@click.option(
    "--clients-dir", help="Directory containing the client's authentication repository"
)
...
@click.option(
    ...
    is_flag=True,
    default=False,
    help="Indicates if the we want to clone a " "repository from the filesystem",
)
...
def setup_test_yubikey():
    ...
    targets_key_path = (
        Path(__file__).parent.parent / "tests" / "data" / "keystore" / "targets"
    )
