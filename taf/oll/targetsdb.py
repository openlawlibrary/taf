...
def create_targets(auth_repo, targets_class=None, base_dir=None, commit=None):
  ...
  if _auth_repo.name not in _targetsdb_dict:
    _targetsdb_dict[_auth_repo.name] = {}
  elif:
    if commit in _targetsdb_dict[_auth_repo.name]:
      return
  ...
  for target_path in targets['signed']['targets']:
    if target_path not in repos:
      continue
    repo_path = os.path.join(self.base_dir, target_path)
