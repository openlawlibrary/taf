...
from taf.tests.test_updater.conftest import KEYS_DESCRIPTION, RepositoryConfig, TaskManager, create_authentication_repository, create_info_json, create_mirrors_json, create_repositories_json, initialize_target_repositories, sign_target_files, sign_target_repositories
...
def origin_auth_repo(request):
    ...
    targets_config = [RepositoryConfig(f"{setup_type}/{targets_config['name']}", targets_config.get("allow_unauthenticated_commits", False))
        for targets_config in targets_config_list]
    repo_name = f"{setup_type}/auth"
    if setup_type == "all_files_initially":
        yield from setup_repository_all_files_initially(repo_name, targets_config)
    elif setup_type == "no_info_json":
        yield from setup_repository_no_info_json(repo_name, targets_config)
    elif setup_type == "mirrors_added_later":
        yield from setup_repository_mirrors_added_later(repo_name, targets_config)
    elif setup_type == "repositories_and_mirrors_added_later":
        yield from setup_repository_repositories_and_mirrors_added_later(repo_name, targets_config)
    elif setup_type == "no_target_repositories":
        yield from setup_repository_no_target_repositories(repo_name, targets_config)
    else:
...
def setup_repository_all_files_initially(repo_name, targets_config):
    ...
    setup_manager.add_task(create_authentication_repository, [{"keys_description": KEYS_DESCRIPTION}])
...
def setup_repository_mirrors_added_later(repo_name, targets_config):
    ...
    setup_manager.add_task(initialize_target_repositories, [{"targets_config": targets_config}])
    setup_manager.add_task(sign_target_repositories, [{"keystore": KEYSTORE_PATH}])
    setup_manager.add_task([create_mirrors_json, sign_target_files], [{}, {"keystore": KEYSTORE_PATH}])
    setup_manager.run_tasks()
...
def setup_repository_repositories_and_mirrors_added_later(repo_name, targets_config):
    ...
    setup_manager.add_task(create_info_json)
    setup_manager.add_task(create_authentication_repository, [{"keys_description": KEYS_DESCRIPTION}])
    setup_manager.add_task([create_repositories_json, create_mirrors_json, sign_target_files],
