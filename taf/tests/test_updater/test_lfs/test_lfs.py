...
from taf.lfs import lfs as lfs_module
from taf.lfs import lfs_process
from taf.exceptions import GitError, GitLFSError
from taf.git import GitRepository
from taf.lfs.lfs import filter_through_git_lfs
...
#: git-lfs' own wording when the server does not have an object it needs.
LFS_UNFETCHABLE_PATTERN = r"Smudge error"
...
def test_an_update_fails_when_the_server_lacks_the_new_objects(
    origin_auth_repo, client_dir, lfs_server
):
    """An update the LFS server cannot supply fails, and says why.

    The clone case is covered above; this is the same gap found later, when the
    client already has the repositories and new commits point at objects the
    ...
    # the second revision's objects never reach the server, so the client
    # cannot fetch what the new commits point at
    ...
    update_invalid_repos_and_check_if_repos_exist(
        ...
        origin_auth_repo,
        client_dir,
        LFS_UNFETCHABLE_PATTERN,
        True,
    )


@needs_git_lfs
def test_a_filter_failure_is_a_git_error(tmp_path):
    """Callers that handle git failures handle this one.

    Every other git error in `taf.exceptions` inherits `GitError`, and the
    updater catches that around the checkouts this filter runs in.
    """
    origin = build_lfs_origin(tmp_path / "origin")
    client = GitRepository(path=tmp_path / "client")
    client.clone_from_disk(origin.path, keep_remote=True)
    ...
    client._git("config lfs.url {}", "http://127.0.0.1:1/lfs")
    for repo_path in (client.path, origin.path):
        shutil.rmtree(repo_path / ".git" / "lfs" / "objects", ignore_errors=True)

    client = GitRepository(path=client.path)
    with pytest.raises(GitError, match="Git LFS could not process"):
        client.checkout_branch(client.default_branch)
...
def test_a_merge_that_cannot_be_filtered_leaves_no_merge_in_progress(tmp_path):
    ...
    client.commit_empty("later")
    client.merge_branch(client.default_branch)
...
def test_a_merge_whose_commit_fails_can_still_be_completed(tmp_path):
    ...
    hook.unlink()
    client.commit("retried")
    parents = run(
        "git", "-C", str(client.path), "rev-list", "--parents", "-1", "HEAD"
    ).split()[1:]
    merged = str(client.top_commit_of_branch(client.default_branch))
...
def test_a_quoted_git_lfs_path_is_recognized(tmp_path, monkeypatch):
    ...
    workdir.mkdir()
    GitRepository(path=workdir).init_repo()
    quoted = f'"{shutil.which("git-lfs")}" filter-process'
    GitRepository(path=workdir)._git("config --local filter.lfs.process {}", quoted)
...
def test_an_unfetchable_object_costs_one_fetch_attempt(tmp_path, lfs_server):
    ...
    client._git("config lfs.url {}", lfs_server.url)
    for repo_path in (client.path, origin.path):
        shutil.rmtree(repo_path / ".git" / "lfs" / "objects", ignore_errors=True)
...
def test_config_changes_take_effect_immediately(tmp_path):
    ...
    for key in ("filter.lfs.process", "filter.lfs.smudge", "filter.lfs.clean"):
        GitRepository(path=workdir)._git(
            "config --local {} {}", key, "some-other-program"
        )
...
def test_filter_agrees_with_git_when_lfs_is_not_configured(tmp_path, lfs_log):
    ...
    client = GitRepository(path=client.path)
    porcelain = client._git("status --porcelain") or ""
...
def test_skip_smudge_is_honored(tmp_path):
    ...
    client.clone_from_disk(origin.path, keep_remote=True)
    client.checkout_branch("other", create=True)
    client._git("lfs install --local --skip-smudge")
